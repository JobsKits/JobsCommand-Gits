...
# - 运行提示：运行后会先打印内置自述；终端模式按回车确认后继续，写入前默认跳过。
...
# 定位需要刷新的 Git 仓库根目录，兼容脚本放在仓库根目录或一脚本目录内。
...
REPO_ROOT="$(resolve_target_repo_root)"
...
show_script_intro_and_wait() {
  ...
  print -r -- '运行策略：先展示扫描结果和 diff；真正写入前直接回车跳过，输入任意字符后才执行。'
  print -r -- "目标仓库：${REPO_ROOT}"
...
print_scan_report() {
  ...
  note_echo "目标 Git 仓库：${REPO_ROOT}"
  note_echo ".gitmodules：${GITMODULES_FILE}"
  echo ""
...
run_business() {
  ...
  echo ""
  if ! ask_any_to_run "确认用上述内容更新 .gitmodules 吗？"; then
